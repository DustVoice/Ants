...
public class AdvancedControlPanel
{
	...
	public AdvancedControlPanel(final Ants ants)
	{
		...
		pheromonePanel
				.setLayout(new BoxLayout(pheromonePanel, BoxLayout.Y_AXIS));
		...
		evaporationPanel.setLayout(new BoxLayout(evaporationPanel,
				BoxLayout.Y_AXIS));
		...
		stepsToMaxPanel.setLayout(new BoxLayout(stepsToMaxPanel,
				BoxLayout.Y_AXIS));
	...
	public void step()
	{
		...
		if (adjustCheckBox.isSelected())
		{
			...
			{
				if (!food.foodPheromoneLevelMap.containsKey(food)
						|| food.foodPheromoneLevelMap.get(food) <= 1.0)
				{
			...
			if (changedTimeout >= stepsToMax && allFoodHasTrail
					&& allNestsHaveTrail)
			{
				maxpheromone -= ((maxMaxpheromone - minMaxpheromone) / stepsToMax)
						* deltapheromone / stepsToMax;
				evaporation -= ((maxEvaporation - minEvaporation) / stepsToMax)
						* deltaEvaporation / stepsToMax;
				dropoff -= ((maxDropoff - minDropoff) / stepsToMax)
						* deltaDropoff / stepsToMax;
				
				chanceOfBestNext += ((maxTrail - minTrail) / stepsToMax)
						* deltaTrail / stepsToMax;
			}
			else
			{
				maxpheromone += ((maxMaxpheromone - minMaxpheromone) / stepsToMax)
