...
public class AntsControlPanel
{
	...
	public AntsControlPanel(final Ants ants,
			final AdvancedControlPanel advancedPanel)
	{
		this.ants = ants;
		...
		antsSlider.setMaximum(5000);
		antsSlider.setMinorTickSpacing(1);
		antsSlider.setMajorTickSpacing(1);
		antsSlider.setSnapToTicks(true);
		antsSlider.setPaintTicks(true);
		
		final JSpinner antCountSpinner = new JSpinner();
		antCountSpinner.addChangeListener(new ChangeListener()
		{
			
			@Override
			public void stateChanged(ChangeEvent e)
			{
				ants.setMaxAnts(Integer.parseInt(antCountSpinner.getValue().toString()));
				ants.repaint(ants.repaint);
			}
			
		});
		antCountSpinner.setValue(1);
		...
		//sizeSpinner.setMinimumSize(controlDimension);
		//sizeSpinner.setMaximumSize(controlDimension);
		sizeSpinner.setPreferredSize(controlDimension);
		//sizeComboBox.addItem("10 X 10");
		/*sizeSpinner.addItem("2");
		sizeSpinner.addItem("3");
		sizeSpinner.addItem("5");
		sizeSpinner.addItem("25");
		sizeSpinner.addItem("50");
		sizeSpinner.addItem("100");
		sizeSpinner.addItem("200");
		sizeSpinner.addItem("500");
		sizeSpinner.addItem("1000");*/
		...
		speedSpinner.setMinimumSize(controlDimension);
		speedSpinner.setMaximumSize(controlDimension);
		speedSpinner.setPreferredSize(controlDimension);
		//speedSpinner.setMinimum(1200);
		//speedSpinner.setMaximum(2100);
		speedSpinner.addChangeListener(new ChangeListener()
		{
			...
			public void stateChanged(ChangeEvent e)
			{
				stepTimer.setDelay(2100 - Integer.parseInt(speedSpinner.getValue().toString()));
			}
			
		});
		speedSpinner.setValue(2100);
		
		JPanel foodRequiredPanel = new JPanel();
		foodRequiredPanel.setLayout(new BoxLayout(foodRequiredPanel,
				BoxLayout.Y_AXIS));
		foodRequiredPanel.setBorder(BorderFactory
				.createTitledBorder("Food Needed"));
		...
		patternComboBox.addItem("Delete Ants");
		patternComboBox.addItem("Delete All");
		patternComboBox.addItemListener(new ItemListener()
		{
			@Override
			public void itemStateChanged(ItemEvent e)
			{
				if (patternComboBox.getSelectedIndex() != -1 && patternComboBox.getSelectedIndex() <3)
				{
					ants.setPattern((Ants.Pattern) patternComboBox
							.getSelectedItem());
					patternComboBox.setSelectedIndex(-1);
				}
				if (patternComboBox.getSelectedIndex() == 3)
				{
					ants.ants.clear();
					
					for(Cell[] c : ants.cellArray)
					{
						for(Cell d : c)
						{
							d.step(0);
				...
				if (patternComboBox.getSelectedIndex() == 4)
				{
					ants.setPattern((Ants.Pattern) Ants.Pattern.Clear);
					ants.ants.clear();
					patternComboBox.setSelectedIndex(-1);	
				}
		...
		sizeSpinner.addChangeListener(new ChangeListener()
		{
			...
			public void stateChanged(ChangeEvent e)
			{
				String size = sizeSpinner.getValue().toString();
				AntsControlPanel.this.ants.setGridSize(Integer.parseInt(size), Integer.parseInt(size));
			}
		});
		
		/*sizeSpinner.addItemListener(new ItemListener()
		{
			...
			public void itemStateChanged(ItemEvent arg0)
			{
				...
				else if (sizeComboBox.getSelectedItem().equals("500"))
				{
					AntsControlPanel.this.ants.setGridSize(500, 500);
				}
